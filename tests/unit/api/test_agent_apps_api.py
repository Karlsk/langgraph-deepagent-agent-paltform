...
def test_patch_subagent_updates_fields_and_bumps_version(client: TestClient) -> None:
    ...
    created = unwrap(client.post("/agent-apps/subagents", json=_subagent_body()), expected_code=201)
...
def test_patch_skill_updates_body_and_bumps_version(client: TestClient) -> None:
    ...
    created = unwrap(client.post("/agent-apps/skills", json=_skill_body()), expected_code=201)
...
def test_skill_generate_returns_draft(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert unwrap(client.get("/agent-apps/skills")) == []
...
def test_get_agent_app_returns_row_or_404(client: TestClient) -> None:
    ...
    app_id = unwrap(client.post("/agent-apps/apps", json=_app_body()), expected_code=201)["id"]
...
def test_patch_agent_app_explicit_null_collection_rejected(client: TestClient, field: str) -> None:
    ...
    assert response.status_code == 422
    assert unwrap(client.get(f"/agent-apps/apps/{app_id}"))["version"] == 1
...
def test_patch_published_app_content_edit_reverts_status_to_draft(client: TestClient) -> None:
    ...
    assert unwrap(client.post(f"/agent-apps/apps/{app_id}/publish"))["status"] == "published"

    response = client.patch(f"/agent-apps/apps/{app_id}", json={"system_prompt": "You are edited."})
    assert response.status_code == 200
    assert unwrap(response)["status"] == "draft"
    assert unwrap(client.get("/agent-apps/apps/published")) == []
...
def _seed_publishable_app(client: TestClient, **app_overrides: Any) -> int:
    ...
    return int(unwrap(client.post("/agent-apps/apps", json=body), expected_code=201)["id"])
...
def test_publish_success_sets_status_hash_and_version(client: TestClient) -> None:
    ...
    listed = unwrap(client.get("/agent-apps/apps/published"))
...
def test_publish_unknown_tool_whitelist_rejected(client: TestClient) -> None:
    ...
    response = client.post(f"/agent-apps/apps/{app_id}/publish")
    assert response.status_code == 422
    assert unwrap(client.get(f"/agent-apps/apps/{app_id}"))["status"] == "draft"
...
def test_publish_subagent_unknown_tool_rejected(client: TestClient) -> None:
    ...
    app_id = unwrap(
        client.post("/agent-apps/apps", json=_app_body(subagent_names=["researcher"], allowed_tools=None)),
        expected_code=201,
    )["id"]
    response = client.post(f"/agent-apps/apps/{app_id}/publish")
    assert response.status_code == 422
...
def test_publish_missing_skill_reference_rejected(client: TestClient) -> None:
    ...
    app_id = unwrap(client.post("/agent-apps/apps", json=_app_body(skill_names=["ghost-skill"])), expected_code=201)[
        "id"
    ]
    response = client.post(f"/agent-apps/apps/{app_id}/publish")
    assert response.status_code == 422
...
def test_publish_missing_subagent_reference_rejected(client: TestClient) -> None:
    ...
    app_id = unwrap(client.post("/agent-apps/apps", json=_app_body(subagent_names=["ghost-sub"])), expected_code=201)[
        "id"
    ]
    response = client.post(f"/agent-apps/apps/{app_id}/publish")
    assert response.status_code == 422
...
def test_publish_missing_llm_config_reference_rejected(client: TestClient) -> None:
    ...
    app_id = unwrap(client.post("/agent-apps/apps", json=_app_body(model="ghost-config")), expected_code=201)["id"]
    response = client.post(f"/agent-apps/apps/{app_id}/publish")
    assert response.status_code == 422
    body = response.json()
...
def test_publish_disabled_llm_config_reference_rejected(client: TestClient) -> None:
    ...
    app_id = unwrap(client.post("/agent-apps/apps", json=_app_body(model="frozen")), expected_code=201)["id"]
    response = client.post(f"/agent-apps/apps/{app_id}/publish")
    assert response.status_code == 422
    body = response.json()
    assert body["code"] == 422
...
def test_patch_llm_config_updates_fields_and_refreshes_hash(client: TestClient) -> None:
    ...
    created = unwrap(client.post("/agent-apps/llm-configs", json=_llm_config_body()), expected_code=201)
...
def test_edit_llm_config_does_not_demote_published_app(client: TestClient) -> None:
    ...
    edited = client.patch(f"/agent-apps/llm-configs/{DEFAULT_LLM_CONFIG_NAME}", json={"api_key": "sk-rotated-0000"})
    assert edited.status_code == 200

    app_payload = unwrap(client.get(f"/agent-apps/apps/{app_id}"))
...
def test_delete_default_agent_app_forbidden(client: TestClient) -> None:
    ...
    app_id = unwrap(client.post("/agent-apps/apps", json=_app_body(name="default")), expected_code=201)["id"]
...
def test_patch_mcp_server_stdio_command_revalidated(client: TestClient) -> None:
    ...
    assert unwrap(client.get("/agent-apps/mcp-servers/fs-server"))["command"] == "uvx"
...
def test_get_and_list_mcp_servers(client: TestClient) -> None:
    ...
    names = [row["name"] for row in unwrap(client.get("/agent-apps/mcp-servers"))]
...
def test_patch_mcp_server_updates_and_invalidates_cache(client: TestClient, quiet_shutdown: AsyncMock) -> None:
    ...
    created = unwrap(client.post("/agent-apps/mcp-servers", json=_mcp_body()), expected_code=201)
...
def test_patch_mcp_server_headers_merge_and_reject_plaintext(client: TestClient) -> None:
    ...
    assert unwrap(client.get("/agent-apps/mcp-servers/fs-server"))["headers"] == {}
    ...
    assert unwrap(client.get("/agent-apps/mcp-servers/fs-server"))["headers"] == {"X-Token": "${X_TOKEN}"}
