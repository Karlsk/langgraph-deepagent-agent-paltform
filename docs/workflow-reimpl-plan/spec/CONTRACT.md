...
- **节点类型白名单**（S18，2026-09-14 **两次**修订）：`PUT` 服务端二次校验 `node.type ∈ {llm, http, python, subworkflow}`；
  `python` 须满足 code-only + `validate_code_ast` 通过 + 服务端强制 `sandboxed=true`（客户端值被忽略并覆写）；
  `subworkflow` 注册期**仅结构校验**（`workflow_id` 为非空 str），被引用存在性为**运行期**检查；其余一律拒绝（详见 S18/S22/S23）。
...
def validate_code_ast(code: str) -> None:
    ...
    规则（命中即拒；消息含**行号 + 规则名**，绝不含代码正文，H6）：
      1. import / from ... import（ast.Import / ast.ImportFrom）——**根模块**须 ∈ _ALLOWED_MODULES，
         否则拒（规则名仍为 `no-import`，消息含**根模块名**）；相对导入（ImportFrom.level > 0）**一律拒**
         （2026-09-14 修订：原为「全禁，无白名单模块」，见 changelog/workflow-designer-portability §2.1）
...
- **`sandbox_worker.py` 不进入任何模块的 import 图**：只被 `run_sandboxed` 以 `[sys.executable, "-I", <绝对路径>]` 拉起；
  协议为 stdin/stdout 各恰好一个 JSON 文档（详见 S22）。**推论（2026-09-14）**：`_ALLOWED_MODULES` 无法由 worker
  `import sandbox` 复用，经 stdin 下发又要改已冻结的协议行 → 两模块**各存一份**，由守护测试以 `ast.literal_eval`
  **从源文件文本**取出比对相等（不 import worker）。
- **R8**：两模块各 < 400 行，函数 < 50 行；AST 规则表、`_ALLOWED_MODULES` 与 `SAFE_BUILTINS` 为模块级常量，便于守护测试直接断言。
...
| HTTP 节点 URL 落入私网/环回/元数据段（SSRF），**字面量 URL** | `WorkflowValidationError` → 422（注册期） | spec-20 |
| 同上，**模板 URL**（`url` 含 `{` 占位符） | `WorkflowValidationError`（**执行期**，对已渲染 URL 抛出）——注册期跳过，见 spec-20 §3 第 4 条 | spec-20 |
| python 节点代码 AST 预检拒绝（**白名单外** import 或相对导入 / dunder / 危险调用 / 超长） | `WorkflowValidationError` → 422 | spec-22 |
...
| 内建白名单 | worker 以 `SAFE_BUILTINS` 作为 `__builtins__`：仅纯计算类（`len/range/sorted/min/max/sum/abs/round/divmod/pow`、`str/int/float/bool/list/dict/set/tuple/frozenset/bytes`、`enumerate/zip/map/filter/reversed/isinstance/issubclass/print`、`chr/ord/hex/oct/bin/format/repr`、`True/False/None`）+ 常用异常类（`Exception/ValueError/TypeError/KeyError/IndexError/AttributeError/ZeroDivisionError/ArithmeticError/LookupError/RuntimeError/StopIteration/NotImplementedError`）+ **受限 `__import__`**（2026-09-14 修订）。**不含** `open/exec/eval/compile/globals/locals/vars/dir/getattr/setattr/delattr/type/input/breakpoint/exit/quit/help`。<br>该 `__import__` 是 worker 内的 `_restricted_import`，**不是** `builtins.__import__`：只放行根模块 ∈ `_ALLOWED_MODULES` 的导入、`level > 0` 直接拒，并在**运行期复查**白名单（纵深防御：调用方可能绕过注册期 AST 校验）。守护测试须断言 `SAFE_BUILTINS["__import__"] is not builtins.__import__` |
| 模块白名单 | `sandbox.py` 与 `sandbox_worker.py` **各存一份** `_ALLOWED_MODULES`（2026-09-14 新增，**18 项**，用户 2026-09-14 拍板）：`json` `re` `math` `statistics` `decimal` `fractions` `random` `datetime` `itertools` `bisect` `heapq` `copy` `string` `textwrap` `base64` `hashlib` `collections` `functools`。判定**只看根模块**（`import a.b` / `from a.b import c` 均取 `a`）；相对导入（`level > 0`）一律拒。规则名仍为 `no-import`，消息含根模块名、不含代码正文（H6）。<br>**为何双份存储**：worker 以 `sys.executable -I` 拉起且**不得进入任何模块的 import 图**（§4.14 约束），经 stdin 下发则要改**已冻结的通信协议行**——故取「双份常量 + 守护测试」，测试以 `ast.literal_eval` **从源文件文本**取出两份比对相等，不 import worker。<br>**明确排除**（理由见 changelog/workflow-designer-portability §3.2）：`os/sys/subprocess/socket/urllib/http/importlib/inspect/ctypes/pickle/logging/sqlite3` 等能力模块；`uuid`（实测模块源码含 `subprocess.Popen`）；`typing`（实测 `get_type_hints` 源码含 `eval(`）；`dataclasses`/`enum`/`time`。<br>**每加一个模块都必须逐个评审其公开与单下划线属性**（规则 3 只封 dunder，实测 `re._compiler` 可达）：安全负担由「一次性全禁」转为「持续性评审」 |
| 无网络 | stdlib 无法做 seccomp 级 syscall 过滤，本期防线是**能力剥夺**：**受限 `__import__` 的白名单不含任何网络模块**（`socket`/`urllib`/`http`/`ftplib`/`smtplib`/`webbrowser`/`asyncio`/`ssl` 全部不在 `_ALLOWED_MODULES`）+ builtins 无 `open`/`getattr` + 空 env + AST 规则 3 封死内省链，使沙箱内代码在语言层面拿不到任何网络入口。生产建议叠加容器网络策略<br>*2026-09-14 修订*：原论证为「AST 全禁 import + builtins 无 `__import__`」，白名单放开后该表述不再成立，保证的**来源**改写在白名单本身——这也是「无网络」从**机制保证**降级为**评审保证**的地方，故上一行要求逐模块评审 |
...
| 2026-09-14 | **设计器可移植性三项修订**：① §4.14 规则 1 由「import 全禁，无白名单模块」改为「**根模块**须 ∈ `_ALLOWED_MODULES`，相对导入（`ImportFrom.level > 0`）一律拒」，规则名**刻意仍为 `no-import`**（既有集成守护卡 `import os` 不改即须继续通过），消息含根模块名、仍不含代码正文（H6）；② S22 细则「内建白名单」行把 `__import__` 从**不含**清单移出、改列为 **受限 `__import__`**（worker 内 `_restricted_import`，**不是** `builtins.__import__`，运行期复查白名单，守护测试须断言 `is not`）；③ S22 细则**新增「模块白名单」行**（18 项 stdlib；`sandbox.py` 与 `sandbox_worker.py` **各存一份**，以 `ast.literal_eval` **从源文件文本**取出比对相等——worker 不得进入任何模块的 import 图、经 stdin 下发则要改已冻结的协议行）；④ S22 细则「无网络」行的保证**来源**改写（原论证「AST 全禁 import + builtins 无 `__import__`」在白名单下不再成立），并显式记录此处「无网络」由**机制保证降级为评审保证**；⑤ §5 场景映射：SSRF 行拆为**字面量 URL**（注册期 422）与**模板 URL**（执行期对已渲染 URL 抛出）两行，AST 预检行的 `import` 限定为「**白名单外** import 或相对导入」；⑥ §4.14 约束补两条（`_ALLOWED_MODULES` 双份存储的**推论**、R8 模块级常量清单加 `_ALLOWED_MODULES`）；⑦ 顺带修正两处遗留：§4.13「节点类型白名单」仍写 3 类（S18 第二次修订后应为 4 类，含 `subworkflow` 的结构校验与运行期存在性），以及冻结异常清单笔误 `LookError` → `LookupError` | §4.13（节点类型白名单）/ §4.14（规则 1 + 约束两条）/ §5（场景映射两行）/ §6（S22 细则三行） | `docs/changelog/workflow-designer-portability/spec-01-contract-change.md`；同步 `docs/changelog/workflow-canvas-orchestration/spec-20-http-ssrf-guard.md` §2（In / Out）与 §3（拦截规则第 4 条改写 + 新增第 5 条模板判定）；同步 `docs/workflow-frontend-spec.md` §5.1（**新增 `http` 节点前端约束**：headers 键值行 + 模板占位符不做前端校验；`python` 面板文案由「无 import」改为「仅白名单 stdlib」+ 前端不复制白名单常量）/ §5.2（模板 URL 的校验时机）/ §9（组件树 + §9.3 表单字段表加 `headers`）/ §12（测试覆盖点 6）/ §14（验收两条）；同步 `docs/workflow-node-development.md` §5.5（S18 条件 ② 的 import 限定 + S22 冻结约定补「受限 `__import__`」与「模块白名单」+ 新增第三条「无网络由机制保证降级为评审保证」） | 纯契约变更，提交 `docs:`；下游代码提交用 `refactor!`（**冻结签名零改动**：`validate_code_ast` / `SandboxLimits` / `run_sandboxed` / `validate_http_url` / `HTTPNodeConfig` 全部不变，只改规则语义与 `api.py` 的调用条件）。**动机**：用户要在设计器里创建 `app/sdn/config/sdn_alert_inspection.yaml`，实测 11 个节点里 **7 个被注册期拒**——4 个 http 节点因 `url` 以 `{sdn_base_url}` 开头致 `urlparse` 取不到 scheme，3 个 python 节点因 `import json` / `import re` / `import random` + `from datetime import datetime`；再加前端 `HttpNodeForm` 无 headers 编辑器（`HTTPNodeConfig.headers` 后端早已存在），三个缺口彼此独立且**任一个都足以阻断**演示。**风险判断**：该 YAML 一直走 `build_registry(--dir)` **磁盘路径**，不过 `api.py` 的校验链，而 `PythonNodeConfig.sandboxed` 默认 `False` → 进程内 `exec` 至今可达；故在 PUT 路径放开 18 项白名单是把一个**已存在的无限制面换成其受限子集**，而非从 0 扩到 18。代价须写明：安全负担由「一次性全禁」转为「**每加一个模块都要逐个评审其公开与单下划线属性**」（规则 3 只封 dunder，实测 `re._compiler` 可达）。**备选方案对比**：机制上 ① **AST 白名单 + worker 受限 `__import__`（采纳）**——协议与签名零改动，既有 YAML 字面 `import json` 直接可用；② 预注入模块名、仍禁 import 语句——现有 YAML 一行都过不了，要兼容就得静默删除 import 语句，比直接拒绝更难查；③ 给受信来源开 `sandboxed=false`——直接复活 S15 的 HTTP RCE。范围上 A 16 项纯计算（否决：数据加工太笨）/ **B 18 项含 `collections`+`functools`（采纳）** / C 全 stdlib 黑名单制（否决：与能力剥夺思路相反且必漏——实测 `uuid` 模块源码含 `subprocess.Popen`、`typing.get_type_hints` 源码含 `eval(`）。**拍板记录**：用户 2026-09-14 选定范围 **B（18 项）**、headers 表单 **A（键值行**，否决 B JSON 文本域——会重蹈 `mock_responses` 覆辙，让前端构造出后端必拒的载荷**）**、节奏为**一次做完三项再演示**。三案影响面对比见 changelog §3 |
