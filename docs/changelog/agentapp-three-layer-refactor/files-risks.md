...
### 2.2 Phase 2：三级 Workspace

| 文件 | 当前职责 | 变更 | 改动量 |
|---|---|---|---|
| `app/services/agents/skills_store.py` | 611 行双存实现（DB+disk） | 新增 `_agent_skill_dir`/`_agent_skill_file`/`materialize_for_agent`/`materialize_to_user_combined`/`_read_agent_dir_skill_names`/`compute_workspace_hash`/`materialize_into_combined_directory` | **大** |
| `app/services/agents/bootstrap.py` | `ensure_default_agent_app` + `_backfill_legacy_sessions` | `ensure_default_agent_workspace` 创建 Agent 层骨架；legacy backfill 保持 | 中 |
| `app/services/agents/runtime.py` | `AgentAppRuntime`/`get_runtime`/`DeepAgentsAppRuntime`/`WorkflowAppRuntime` | 装配时新增 Agent 层 skill 复制；chatbot 启动校验 User 层与 Agent 层 drift | 中 |
| `app/services/agents/assembly.py` | `compile_agent_app`/`compile_standalone_subagent`/`compute_fingerprint` | fingerprint 范围新增 `workspace_hash` 字段 | 中 |
| `app/services/agents/test_runner.py` | `run_subagent_once` | 改用 `materialize_into_combined_directory`（Global + Agent 聚合） | 小 |
| `app/api/v1/apps.py` | AgentApp CRUD + publish | publish 流程新增 "Global → Agent 复制" 步骤；新增 `POST /apps/{id}/associate-user/{uid}` 端点 | 中 |
| `app/models/agent_assets.py` | `SkillAsset`/`SubAgentConfig`/`AgentApp`/`McpServerConfig` | `SkillAsset.scope`；`AgentApp.agent_dir`/`workspace_hash`/`agent_workspace_status` | 中 |
| `app/core/config.py` | Pydantic Settings | 新增 `AGENTS_ROOT`（默认 `SKILLS_ROOT / "agents"`） | 小 |
| `alembic/versions/` 新增 | — | 至少 1 个迁移：`add_agent_workspace_fields` | 中 |
...
### 3.2 Phase 2：三级 Workspace

| 文件 | 变更 | 改动量 |
|---|---|---|
| `agent-web/src/views/agent/AgentList.vue`（stub） | 扩展为支持"绑定到用户"操作（按钮 + confirm 对话框） | 中 |
| `agent-web/src/api/apps.ts` | 新增 `associateUserWithApp(appId, userId)` | 小 |
| `agent-web/tests/api/apps.spec.ts` | 新增 associate-user 测试 | 小 |
...
### 4.1 Phase 2 新增迁移（`alembic/versions/<rev>_agent_workspace.py`）

```python
def upgrade():
    # SkillAsset.scope
    op.add_column("skill_asset", sa.Column("scope", sa.String(16), nullable=False, server_default="global"))
    op.create_index("ix_skill_asset_scope", "skill_asset", ["scope"])

    # AgentApp.agent_dir / workspace_hash / agent_workspace_status
    op.add_column("agent_app", sa.Column("agent_dir", sa.String(255), nullable=True))
    op.add_column("agent_app", sa.Column("workspace_hash", sa.String(64), nullable=True))
    op.add_column("agent_app", sa.Column("agent_workspace_status", sa.String(16), nullable=False, server_default="pending"))
    ...
    for row in conn.execute(sa.text("SELECT id, skill_names FROM agent_app")):
        conn.execute(
            ...
            {"dir": f"{settings.AGENTS_ROOT}/{row.id}/skills", "id": row.id},
        )
...
| **R2** | refresh_token DB 哈希泄漏后被批量反推 raw token | 低 | Phase 1 长期 | sha256 哈希存储 + 64 字符 base64 raw token（熵 384 位）→ DB 泄漏无法反推；refresh 旋转 + 重放检测兜底；refresh_token 定期清理（expires_at < now - 7d） |
| **R3** | 三级 Workspace 引入后，Agent 层复制时机不清导致 User Chat 看到过期文件 | 中 | Phase 2 | 严格定义：**publish 时**生成 Agent 层内容；**用户首次绑定 AgentApp 时**复制 Agent → User；后续仅在 Agent 内容变更 + 重新 publish 时增量同步；DB 中以 `workspace_hash` 锁定快照；启动时校验 drift 并日志告警 |
| **R4** | Agent 层命名空间与 AgentApp id 关联后，跨 AgentApp 共享 Skill 文件冲突 | 中 | Phase 2 | Agent 层 Skill 物理隔离：`{AGENTS_ROOT}/<app_id>/skills/<name>/SKILL.md`；不允许两个 AgentApp 共享同一 Agent 层 skill（除非显式引用 Global） |
| **R5** | 现有 AgentApp 行 `agent_dir` 为 NULL 时，新代码如何处理 | 中 | Phase 2 启动期 | alembic 迁移时为所有已发布应用填充 `agent_dir = "{AGENTS_ROOT}/<id>/skills"`；启动时 `ensure_default_agent_workspace` 检查并补齐缺失目录；日志记录 `agent_workspace_backfilled` |
...
- `app/main.py` lifespan 启动流程（仅在 Phase 2 增加 `ensure_default_agent_workspace` 调用）
- `app/core/mcp_client.py`（MCP 会话池 worker 模型，独立演进）
- `app/services/agents/assembly.py` 的 deepagents 编译逻辑（仅 fingerprint 新增字段）
- `app/workflow/` 全部模块（Workflow 引擎单独 spec）
- `alembic/versions/b25d38b0cd7c_initial_schema.py` 既有迁移
- 前端 `agent-web/src/components/WebAgentTable.vue` 与 `WebAgentFormDialog.vue`（通用组件）
- 前端 `agent-web/src/styles/` 与 `agent-web/src/composables/useRequest.ts`（基础设施）
