...
- `tests/unit/workflow/nodes/test_python_node.py` 追加：`sandboxed=true` 路由到 `run_sandboxed`（monkeypatch 计数）；`sandboxed=false` 仍走进程内 `exec`（回归保护）；`entry` + `sandboxed=true` → `ValidationError`；日志摘要含 `sandboxed` 且不含代码正文。该文件原**缺任何 unit 标记**（既无模块级 `pytestmark`，也无逐函数 `@pytest.mark.unit`），本次补上模块级标记使其真正进入 `-m unit` 门禁。**更正一处先前的误判**：曾记为「`tests/unit/workflow/nodes/` 全目录同此历史漂移」，实测（`--collect-only` 带/不带 `-m unit` 对比计数）表明同目录的 `test_base.py`/`test_factory.py`/`test_http_node.py`/`test_llm_node.py` 均用**逐函数** `@pytest.mark.unit`，一直在门禁内（如 `test_llm_node.py` 40/40）；本仓库两种标记风格并存，故 `grep -L pytestmark` 会给出误导性的「未入门禁」清单，判断须以采集计数为准。真正缺标记的只有 `test_python_node.py` 一个文件。
